...
デザインは [docs/original-prototypes/](docs/original-prototypes/) にある5つのReactプロトタイプ（プロフィール／グループチャット／募集投稿／テーマ選択／通報・ブロック）を、実データで動くページとして実装しています。
...
- **安全設計**（5章）: 募集の定員3人以上をUI・APIの両方で強制（`src/lib/rules.ts`）、オフライン注意文、安全ガイドライン同意必須、主催者の信頼スタンプ表示
- **通報・ブロック**（5章・8章）: グループチャットの各メッセージから通報・ブロックできる（`src/components/ReportBlockMenu.tsx`）。ブロックすると、お互いのメッセージ・リアクションが表示/送信できなくなる（`src/lib/blocks.ts`）。運営は `/admin/reports` で未対応の通報を確認・対応できる
